...
- **파일 다양성 조정 (#16)**: RRF로 순위를 매긴 뒤, 같은 파일에서 나온 chunk는 최대 `max_per_file`(기본 2)개까지만 후보 풀에 남긴다. 하나의 파일이 질문과 강하게 매칭되면 상위 슬롯을 그 파일의 여러 chunk가 독점해서 다른 파일의 근거가 누락될 수 있기 때문이다. 특정 파일을 명확히 지목한 질문에는 `max_per_file=None`으로 제한을 끌 수 있다 (질의 유형 분류 #17에서 활용 예정).
